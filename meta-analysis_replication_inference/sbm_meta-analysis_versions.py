...
def meta_scon_pool_init(I_S, I_D, varying_system_parameters, constant_system_parameters, C_t0):
    ...
    if len(varying_system_parameters) == 7 and len(constant_system_parameters) == 1 and len(C_t0) == 3:
        ...
        raise Exception('Mismatch(es) in length(s) of vector(s)/array(s)/tensor(s) corresponding to varying system parameters, constant system parameters, or initial conditions C_t0 as expected for Xie et al., 2020 steady state equation re-arrangement of SCON system')

def meta_sawb_pool_init(I_S, I_D, varying_system_parameters, constant_system_parameters, C_t0):
    '''
    Returns vector of values for SAWB parameters which depend on on proposed values for varying parameters, previously chosen values for other constant parameters, and choices of initial pre-warming steady state pool sizes, C_t0. This function assumes that the parameters that are varied and held constant are the same as for the inferences done in Xie et al., 2020. In Xie et al., 2020, steady state equations were re-arranged so that rather than pre-warming steady state pool sizes following from varying parameter values, pool sizes were pre-emptively set, which required some parameter values to be a priori held constant and others to be calculated from the re-arranged steady state equations following sampling of values for the varying system parameters at each Markov chain step. This was done to biologically constrain steady state pool sizes to biologically realistic values.
    Dictionaries expected for varying_system_parameters and constant_system_parameters. Vector/array/tensor expected for C_t0.
    '''
    if len(varying_system_parameters) == 9 and len(constant_system_parameters) == 1 and len(C_t0) == 4:
        ...
        raise Exception('Mismatch(es) in length(s) of vector(s)/array(s)/tensor(s) corresponding to varying system parameters, constant system parameters, or initial conditions C_t0 as expected for Xie et al., 2020 steady state equation re-arrangement of SAWB system.')

# def sbm_pool_init_meta(I_S, I_D, varying_system_parameters, constant_system_parameters, C_t0, SDE_system):
#     '''
...
#             return {'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref}
#         else:
#             raise Exception('Mismatch(es) in length(s) of vector(s)/array(s)/tensor(s) corresponding to varying system parameters, constant system parameters, or initial conditions C_t0.')
...
#             raise Exception('Mismatch(es) in length(s) of vector(s)/array(s)/tensor(s) corresponding to varying system parameters, constant system parameters, or initial conditions C_t0.')
#     else:
#         raise Exception('Ineligible model provided. "SCON" and "SAWB" only for now.')
